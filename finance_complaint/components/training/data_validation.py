...
class DataValidation(FinanceDataSchema):
    ...
    #         logging.info(f"Number of row: {total_rows} ")
    #
    #         # Drop row if target value is unknown
    #         logging.info(f"Dropping rows without target value.")
    ...
    #                                                                                   "unknown"))
    #
    #         logging.info(f"Unlabelled data has row: [{unlabelled_dataframe.count()}] and columns:"
    #                     f" [{len(unlabelled_dataframe.columns)}]")
    #
    #         logging.info(f"Write unlabelled data into rejected file path: [{file_path}]")
    #         unlabelled_dataframe.write.mode("append").parquet(file_path)
    #
    #         dataframe: DataFrame = dataframe.filter(f"{self.target_column}!= 'N/A'")
    #
    #         logging.info(f"Remaining data has rows: [{dataframe.count()}] and columns: [{len(dataframe.columns)}]")
    #         return dataframe
    #     except Exception as e:
    #         raise FinanceException(e, sys)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            logging.info("Saving validated data.")
